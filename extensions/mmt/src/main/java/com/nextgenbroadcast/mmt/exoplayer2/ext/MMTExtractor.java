package com.nextgenbroadcast.mmt.exoplayer2.ext;
...
public class MMTExtractor implements Extractor, IAtsc3NdkMediaMMTBridgeCallbacks {
    ...
    public static long SystemClockAnchor = 0L;
    public static long MfuClockAnchor = 0L;
    ...
    private final HashMap<MMTAudioDecoderConfigurationRecord, Boolean> audioConfigurationMap = new HashMap<>();
    ...
    private long stppMfuPresentationTimestampUs;

    public MMTExtractor() {
        //jjustman-2021-01-04 - workaround as renderer's don't get a reference to extractor...
        SystemClockAnchor = 0;
    }
    ...
    public void release() {
        ...
        atsc3NdkMediaMMTBridge.release();
    }
...
//        }
    ...
    private void maybeOutputFormat() {
        if (!hasOutputFormat) {
            ...
            TrackOutput videoOutput = MMTMediaTrackUtils
                .createVideoOutput(extractorOutput, /* id */1, videoType, videoWidth, videoHeight, videoFrameRate, initBuffer.array());
            if (videoOutput != null) {
                ...
                for (Map.Entry<MMTAudioDecoderConfigurationRecord, Boolean> entry : audioConfigurationMap.entrySet()) {
                    if (!entry.getValue()) {
                        ...
                        if (record.audioAC4SampleEntryBox != null) {
                            TrackOutput audioOutput = MMTMediaTrackUtils
                                .createAudioOutput(extractorOutput, record.packet_id, record.audioAC4SampleEntryBox.type, record.channel_count, record.sample_rate);
                            if (audioOutput != null) {
                                entry.setValue(true);
            ...
            TrackOutput textOutput = MMTMediaTrackUtils
                .createTextOutput(extractorOutput, /* id */3, textType);
            if (textOutput != null) {
    ...
    private void addVideoFragment(MfuByteBufferFragment mfuByteBufferFragment) {
        ...
        //normal flow...
        if(mfuBufferQueue == null) {
            Log.e("addAudioFragment", "mfuBufferQueue is NULL!");
            return;
        }
...
//            }
    ...
    private void addSubtitleFragment(MfuByteBufferFragment mfuByteBufferFragment) {
        ...
        if(mfuBufferQueue == null) {
            Log.e("addSubtitleFragment", "mfuBufferQueue is NULL!");
            return;
        }
