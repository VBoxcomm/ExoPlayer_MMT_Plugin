...
/**
 * Based on ProgressiveMediaPeriod
 */
public final class MMTMediaPeriod
        implements MediaPeriod,
        ExtractorOutput,
        Loader.Callback<MMTMediaPeriod.ExtractingLoadable>,
...
//    private final ConditionVariable loadCondition;
    ...
    public MMTMediaPeriod(
        ...
        loader = new Loader("Loader:MMTMediaPeriod");
                ...
                () -> {
                    if (!released) {
                        Assertions.checkNotNull(callback)
                                .onContinueLoadingRequested(this);
                    }
                };
        handler = new Handler();
    ...
    private void startLoading() {
        ExtractingLoadable loadable =
                new ExtractingLoadable(
                        uri, dataSource, extractorHolder, /* extractorOutput= */ this, null
//                        loadCondition
                );
        if (prepared) {
            SeekMap seekMap = getPreparedState().seekMap;
    ...
    private final class SampleStreamImpl implements SampleStream {
        ...
        //jjustman-2020-12-23 - super hacky
        private boolean hasPreviouslySetIsReady = false;
        ...
        public boolean isReady() {
            boolean trackIsReady = hasPreviouslySetIsReady || MMTMediaPeriod.this.isReady(track);
            hasPreviouslySetIsReady = trackIsReady;
            return trackIsReady;
        }

        @Override
        public void maybeThrowError() throws IOException {
            MMTMediaPeriod.this.maybeThrowError();
        }

        @Override
        public int readData(FormatHolder formatHolder, DecoderInputBuffer buffer,
                            boolean formatRequired) {
            return MMTMediaPeriod.this.readData(track, formatHolder, buffer, formatRequired);
        }

        @Override
        public int skipData(long positionUs) {
            return MMTMediaPeriod.this.skipData(track, positionUs);
        }
