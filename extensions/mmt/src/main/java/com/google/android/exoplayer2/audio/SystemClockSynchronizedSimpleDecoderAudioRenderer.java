 ...
 * </ul>
 */
public abstract class SystemClockSynchronizedSimpleDecoderAudioRenderer extends BaseRenderer implements MediaClock {


  public long JJOutputBufferTimeUs;
  ...
  public SystemClockSynchronizedSimpleDecoderAudioRenderer() {
  ...
  public void render(long positionUs, long elapsedRealtimeUs) throws ExoPlaybackException {
    ...
    if (decoder != null) {
      try {
        ...
        Log.i("SystemClockSynchronizedSimpleDecoderAudioRenderer",String.format("JJ: before drainOutputBuffer, inputBufferCount: %d, outputBufferCount: %d, diff: %d", decoderCounters.inputBufferCount, decoderCounters.outputBufferCount, (decoderCounters.outputBufferCount - decoderCounters.inputBufferCount)));
        while (drainOutputBuffer()) {}
        Log.i("SystemClockSynchronizedSimpleDecoderAudioRenderer",String.format("JJ: after drainOutputBuffer: outputBufferCount: %d, renderedOutputBufferCount: %d, ", decoderCounters.outputBufferCount, decoderCounters.renderedOutputBufferCount));

        while (feedInputBuffer()) {}
        Log.i("SystemClockSynchronizedSimpleDecoderAudioRenderer",String.format("JJ: after feedInputBuffer, inputBufferCount: %d", decoderCounters.inputBufferCount));
  ...
  private boolean drainOutputBuffer() throws ExoPlaybackException, AudioDecoderException,
    ...
    if (outputBuffer == null) {
      ...
      if (outputBuffer == null) {
        Log.i("SystemClockSynchronizedSimpleDecoderAudioRenderer",String.format("JJ: decoder.drainOutputBuffer returned NULL! decoder: %s, last decoderCounters.outputBufferCount: %d", decoder, decoderCounters.outputBufferCount));
        return false;
      }
      decoderCounters.outputBufferCount++;
    ...
    Log.i("SystemClockSynchronizedSimpleDecoderAudioRenderer",String.format("JJ: drainOutputBuffer, getPositionUs(): %d, outputBufferInfo.presentationTimeUs: %d, codec: %s, decoderCounters.outputBufferCount: %d, decoderCounters.renderedOutputBufferCount: %d",
            getPositionUs(),
            outputBuffer.timeUs, this, decoderCounters.outputBufferCount, decoderCounters.renderedOutputBufferCount));
    ...
    if((outputBuffer.timeUs - myPositionUs) > 66000) {
      ...
      if (audioSink.handleBuffer(outputBuffer.data, outputBuffer.timeUs)) {
        ...
        JJOutputBufferTimeUs = outputBuffer.timeUs;
  ...
  private boolean feedInputBuffer() throws AudioDecoderException, ExoPlaybackException {
    ...
    if (inputBuffer == null) {
      ...
      if (inputBuffer == null) {
        Log.i("SystemClockSynchronizedSimpleDecoderAudioRenderer",String.format("JJ: decoder.dequeueInputBuffer returned NULL! decoder: %s", decoder));
        return false;
      }
    }
  ...
  //jjustman-2021-01-04 - TODO: update this against our MMTExtractor anchor time if we have not been updated
  @Override
  public long getPositionUs() {
    if (getState() == STATE_STARTED) {
      updateCurrentPosition();
    }
    //jjustman-2021-01-04 - base this upon our decode latency output + 750000L for buffering
    return ((System.currentTimeMillis() - MMTExtractor.SystemClockAnchor) * 1000) - 750000L;
  }
...
//        currentPositionUs = buffer.timeUs;
//        Log.d("SystemClockSynchronizedSimpleDecoderAudioRenderer", String.format("JJ: onQueueInputBuffer - forcing allowFirstBufferPositionDiscontinuity: %b, currentPositionUs: %d, buffer.timeUs: %d", allowFirstBufferPositionDiscontinuity, currentPositionUs, buffer.timeUs));
//      }
//      allowFirstBufferPositionDiscontinuity = false;
//    } else {
//      Log.d("SystemClockSynchronizedSimpleDecoderAudioRenderer", String.format("JJ: onQueueInputBuffer - allowFirstBufferPositionDiscontinuity: %b, currentPositionUs: %d, buffer.timeUs: %d (differential: %d uS | %f s)", allowFirstBufferPositionDiscontinuity, currentPositionUs, buffer.timeUs, (buffer.timeUs - currentPositionUs), (buffer.timeUs - currentPositionUs)/1000000.0));
//    }
  }

  private void updateCurrentPosition() {
    Log.i("SystemClockSynchronizedSimpleDecoderAudioRenderer",String.format("JJ: updateCurrentPosition - JJOutputBufferTimeUs: %d, last currentPositionUs: %d (diff: %f s)", JJOutputBufferTimeUs, currentPositionUs, (JJOutputBufferTimeUs - currentPositionUs) / 1000000.0));

    currentPositionUs = JJOutputBufferTimeUs;

//    long lastCurrentPositionUs = currentPositionUs;
...
//      Log.d("SimpleDecoderAudioRenderer",String.format("JJ: updateCurrentPosition with allowPositionDiscontinuity: %b: newCurrentPositionUs: %d, currentPositionUs: %d, lastCurrentPositionUs: %d", allowPositionDiscontinuity, newCurrentPositionUs, currentPositionUs, lastCurrentPositionUs));
//      allowPositionDiscontinuity = false;
//    } else {
//      Log.d("SimpleDecoderAudioRenderer",String.format("JJ: updateCurrentPosition: ignoring currentPositionUs: %d, lastCurrentPositionUs: %d", currentPositionUs, lastCurrentPositionUs));
//    }
  ...
  private final class AudioSinkListener implements AudioSink.Listener {
    ...
    public void onAudioSessionId(int audioSessionId) {
      ...
      SystemClockSynchronizedSimpleDecoderAudioRenderer.this.onAudioSessionId(audioSessionId);
    }
    ...
    public void onPositionDiscontinuity() {
      ...
      SystemClockSynchronizedSimpleDecoderAudioRenderer.this.allowPositionDiscontinuity = true;
    }

    @Override
    public void onUnderrun(int bufferSize, long bufferSizeMs, long elapsedSinceLastFeedMs) {
      Log.w("SystemClockSynchronizedSimpleDecoderAudioRenderer",String.format("JJ: not dispatching onUnderrun: bufferSize: %d, bufferSizeMs: %d, elapsedSinceLastFeedMs: %d", bufferSize, bufferSizeMs, elapsedSinceLastFeedMs));
