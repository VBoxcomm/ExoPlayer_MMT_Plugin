...
/* package */ final class AudioTrackPositionTracker {
  ...
  public boolean mayHandleBuffer(long writtenFrames) {
    ...
    if (hadData && !hasData && playState != PLAYSTATE_STOPPED && listener != null) {
      Log.w("AudioTrackPositionTracker", String.format("JJ - NOT invoking listener.onUnderrun: bufferSize: %d, bufferSizeMS: %d", bufferSize, C.usToMs(bufferSizeUs)));
      //jjustman-2020-11-19 - hack -
        //Log.w("AudioTrackPositionTracker", String.format("JJ - invoking listener.onUnderrun: bufferSize: %d, bufferSizeMS: %d", bufferSize, C.usToMs(bufferSizeUs)));
   ...
   */
  private static Integer __COUNTER_getPlaybackHeadPosition = 0;
  ...
  private long getPlaybackHeadPosition() {
    ...
    if (state == PLAYSTATE_STOPPED) {
        if((__COUNTER_getPlaybackHeadPosition++ % 100) == 0) {
